...
There are two types of clickable elements, buttons and links.

Links are used when you need to direct someone to a different page on a website, and buttons when you
want to perform an action when something is clicked.

Sometimes developers try to implement their own button without using the built-in button element in HTML so it can
